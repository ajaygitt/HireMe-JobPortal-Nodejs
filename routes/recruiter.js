...
router.get("/myJobs", verifyLoggedIn, verifyIfPremium, (req, res) => {
  ...
  let myJobs = recruiterHelper.myJobs(userfound._id).then((myJobs) => {
    ...
    res.render("recruiter/manageJobs", {
      recruiter: true,
      viewonly,
      myJobs,
      userfound,
...
router.get("/browse-employees", verifyIfPremium, verifyLoggedIn, (req, res) => {
  ...
  recruiterHelper.getAllUsers().then((users) => {
    res.render("recruiter/browse-employees", {
      recruiter: true,
      users,
      userfound,
    });
...
router.post("/editJob", verifyLoggedIn, verifyLoggedIn, (req, res) => {
  let id = req.query.job;
...
router.get("/viewUserResume", verifyIfPremium, verifyLoggedIn, (req, res) => {
  ...
  recruiterHelper.viewMyProfile(applicant).then((applicant) => {
    ...
    res.render("recruiter/viewResume", {
      recruiter: true,
      applicant,
      jobId,
      applicant,
...
router.post("/approveJob", verifyIfPremium, verifyLoggedIn, (req, res) => {
  ...
  recruiterHelper
    .approveApplicant(req.body.user, req.body.job)
    .then((response) => {
      notificationHelper.sendApprovedNotification(
        req.body.user,
        req.body.job,
        userfound._id
      );
...
router.post("/reject", verifyIfPremium, verifyLoggedIn, (req, res) => {
  let userfound = req.session.user;
  recruiterHelper
    .rejectApplicant(req.body.user, req.body.job)
    .then((response) => {
      notificationHelper.sendRejectedNotification(
        req.body.user,
        req.body.job,
        userfound._id
      );
