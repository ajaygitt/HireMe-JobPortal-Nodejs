...
module.exports = {
  ...
  sendChat: (sender, receiver) => {
    return new Promise(async (resolve, reject) => {
      let messages = await db
        ...
        .find({
          $and: [
            { sender_id: ObjectID(sender), receiver_id: ObjectID(receiver) },
          ],
        })
        .toArray();
  ...
  receivedChat: (sender, reciever) => {
    ...
    return new Promise(async (resolve, reject) => {
      let messageis = await db
        .get()
        .collection(collection.MESSAGE_COLLECTION)
        .find({
          $and: [
            { sender_id: ObjectID(sender), receiver_id: ObjectID(reciever) },
          ],
        })
        .toArray();
      resolve(messageis);
